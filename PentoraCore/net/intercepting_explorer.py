...
from PentoraCore.net.cookies import mitm_jar_to_cookiejar
...
from PentoraCore.net.async_stickycookie import AsyncStickyCookie
from PentoraCore.net.explorer import Explorer, EXCLUDED_MEDIA_EXTENSIONS
from PentoraCore.net.scope import Scope, wildcard_translate
from PentoraCore.main.log import log_verbose, log_blue, logging, log_blue, log_blue
...
def mitm_to_pentora_request(mitm_request: MitmRequest) -> Optional[Request]:
    post_params = []
    enctype = ""
...
class MitmFlowToPentoraRequests:
    ...
    async def response(self, flow):
        ...
        if "text" in content_type or "json" in content_type or "html" in content_type or "xml" in content_type:
            request = mitm_to_pentora_request(flow.request)
            if request is None:
                return
...
async def launch_proxy(
    ...
    # Finally here is our custom addon that will generate Pentora Request and Response objects and push them to the queue
    master.addons.add(MitmFlowToPentoraRequests(data_queue, headers, scope, drop_cookies))
    try:
        await master.run()
