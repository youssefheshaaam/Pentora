...
class SqlPersister:
    ...
    BASE_DIR = os.path.join(HOME_DIR, ".pentora")
    ...
    async def count_payloads(self) -> int:
        """
        Count the total number of payloads (vulnerabilities, anomalies, and additionals).
        
        Returns:
            The total number of payloads
        """
        statement = select(sql_count(self.payloads.c.response_id))
        async with self._engine.begin() as conn:
            result = await conn.execute(statement)
            return result.fetchone()[0]
    ...
    async def load_scan_state(self):
        """Load the scan state from the database"""
        # This method is called when resuming a scan
        # For now, it's just a placeholder to satisfy the interface
        pass

    async def get_vulnerabilities(self):
        """Return a dictionary of all vulnerabilities found during the scan."""
        vulnerabilities = {}
        
        async with self._engine.begin() as conn:
            # Join with paths and responses to get more information
            query = select(
                self.payloads,
                self.paths.c.path,
                self.paths.c.method,
                self.paths.c.headers,
                self.paths.c.referer,
                self.responses.c.status_code,
                self.responses.c.headers.label("response_headers"),
                self.responses.c.body
            ).select_from(
                self.payloads.join(
                    self.paths,
                    self.payloads.c.evil_path_id == self.paths.c.path_id
                ).join(
                    self.responses,
                    self.payloads.c.response_id == self.responses.c.response_id,
                    isouter=True
                )
            ).where(
                self.payloads.c.type == "vulnerability"
            )
            
            result = await conn.execute(query)

        for row in result.fetchall():
            evil_id, original_id, module, category, level, parameter, info, payload_type, wstg, response_id = row[:10]
            path, method, headers, referer = row[10:14]
            status_code, response_headers, body = row[14:17]

            if category not in vulnerabilities:
                vulnerabilities[category] = []
            
            # Create a request object for the vulnerability
            request_info = {
                "url": path,
                "method": method,
                "headers": headers,
            ...
            # Create a response object for the vulnerability
            response_info = {
            ...
            vulnerabilities[category].append({
                ...
                "info": info,
                "type": payload_type,
                "wstg": json.loads(wstg) if wstg else None,
    ...
    async def get_additionals(self):
        """Return a dictionary of all additional information found during the scan."""
        additionals = {}
        ...
        async with self._engine.begin() as conn:
            ...
            query = select(
                ...
                self.payloads.c.type == "additional"
            )
        ...
        for row in result.fetchall():
            ...
            if category not in additionals:
                additionals[category] = []
            
            # Create a request object for the additional info
            request_info = {
                "url": path,
                "method": method,
                "headers": headers,
            ...
            # Create a response object for the additional info
            response_info = {
                "status_code": status_code,
                "headers": response_headers,
                "body": body
