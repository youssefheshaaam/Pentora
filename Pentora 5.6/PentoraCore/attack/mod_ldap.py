# Module Description:
# This module detects LDAP injection vulnerabilities by testing parameters that
# might be used in LDAP queries. LDAP injection can allow attackers to bypass
# authentication, access unauthorized information, or modify LDAP queries to
# extract sensitive data from directory services.
import dataclasses
import html
...
from PentoraCore.definitions.ldapi import LdapInjectionFinding
...
def find_ldap_error(text: str) -> Optional[str]:
    log_verbose(f"Checking for LDAP errors in text: {text}")
    for error_message in LDAP_ERRORS:
        if error_message in text:
            log_verbose(f"Found LDAP error: {error_message}")
            return error_message
    log_verbose("No LDAP errors found")
    return None
...
class ModuleLdap(Attack):
    ...
    async def attack_parameter(
        ...
        if warn_request:
            ...
            # Special check for Pentora Vulnerable Lab LDAP section
            if "vulnerabilities/ldap" in original_request.url:
                ...
                log_red(Messages.MSG_EVIL_REQUEST)
                log_red(http500_request.http_repr())
                log_red("---")
                return True
